...
function Comment({ _id, author, text, isSpoil, createdAt, onSmash }) {
  return (
        ...
        <div className="mt-1 text-text-secondary relative">
          ...
          {isSpoil ? (
            <div className="absolute -inset-2 flex justify-center items-center backdrop-blur-xs">
              <button
                onClick={() => onSmash(_id)}
                className="hover:border-cta-primary/50 hover:bg-cta-primary/50 bg-input-border/50 backdrop-blur-3xl transition-colors duration-300 cursor-pointer border-2 text-sm border-input-border rounded-xl px-3 py-1 text-text-primary remove-comment-spoiler-cover-btn"
              >
                Read
              </button>
            </div>
